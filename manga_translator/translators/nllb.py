...
class NLLBTranslator(OfflineTranslator):
    ...
    async def _load(self, from_lang: str, to_lang: str, device: str):
        ...
        if ':' not in device:
            device += ':0'
        self.device = device
        self.model = AutoModelForSeq2SeqLM.from_pretrained(self._TRANSLATOR_MODEL).to(device)
    ...
    def _translate_sentence(self, from_lang: str, to_lang: str, query: str) -> str:
        ...
        # Use direct model inference instead of pipeline('translation')
        ...
        if ':' in device:
            ...
            import torch
            inputs = {k: v.to(device + ':0') for k, v in inputs.items()}

        # Resolve forced BOS token for target language
        forced_bos_id = self.tokenizer.convert_tokens_to_ids(to_lang)
        if forced_bos_id == self.tokenizer.unk_token_id:
            sp_map = getattr(self.tokenizer, "added_tokens_encoder", {})
            forced_bos_id = sp_map.get(to_lang, forced_bos_id)
